...
class ReflACTTrainer:
    ...
    def train(self) -> dict:
        ...
        # Pass full config to model layer for simple_openai support
        from skillopt.model import set_optimizer_config, set_target_config
        set_optimizer_config(cfg["optimizer_model"])
        set_target_config(cfg["target_model"])

        opt_model = cfg["optimizer_model"]
        if isinstance(opt_model, dict):
            opt_model = opt_model.get("model", "unknown")
        set_optimizer_deployment(opt_model)

        target_model = cfg["target_model"]
        if isinstance(target_model, dict):
            target_model = target_model.get("model", "unknown")
        ...
        if os.path.exists(skill_init_path):
            with open(skill_init_path, "r", encoding="utf-8") as f:
                skill_init = f.read()
