...
from skillopt.model import simple_openai as _simple

# Global config storage for simple_openai
_OPT_CFG = {}
_TARGET_CFG = {}

def set_optimizer_config(cfg: dict) -> None:
    global _OPT_CFG
    _OPT_CFG = cfg

def set_target_config(cfg: dict) -> None:
    global _TARGET_CFG
    _TARGET_CFG = cfg
...
def chat_optimizer(
    ...
    if get_optimizer_backend() == "openai_chat" and _OPT_CFG:
        client = _simple.get_optimizer_client(_OPT_CFG)
        return _simple.chat(client, _OPT_CFG["model"], system, user, max_completion_tokens, retries)
...
def chat_target(
    ...
    if get_target_backend() == "openai_chat" and _TARGET_CFG:
        client = _simple.get_target_client(_TARGET_CFG)
        return _simple.chat(client, _TARGET_CFG["model"], system, user, max_completion_tokens, retries)
