...
def _make_client(role: str) -> AzureOpenAI | OpenAI:
    ...
    # If it's an API Key and doesn't look like an Azure endpoint, use standard OpenAI client
    if auth_mode in {"api_key", "key"}:
        if not cfg["api_key"]:
            raise ValueError(
                f"API key is not configured for {role}. "
                "Set model.api_key in the config or export AZURE_OPENAI_API_KEY."
            )
        
        endpoint = cfg["endpoint"]
        if endpoint and not ("openai.azure.com" in endpoint or "cognitiveservices.azure.com" in endpoint):
            return OpenAI(
                base_url=endpoint,
                api_key=cfg["api_key"],
            )
...
def get_optimizer_client() -> AzureOpenAI | OpenAI:
    ...
    with _optimizer_lock:
        if _optimizer_client is None:
            # If endpoint is set but not an Azure one, use standard OpenAI
            from skillopt.model.backend_config import get_optimizer_backend
            if get_optimizer_backend() == "openai_chat":
                endpoint = os.environ.get("OPTIMIZER_AZURE_OPENAI_ENDPOINT") or ENDPOINT
                if endpoint and not ("openai.azure.com" in endpoint or "cognitiveservices.azure.com" in endpoint):
                    _optimizer_client = OpenAI(
                        base_url=endpoint,
                        api_key=os.environ.get("OPTIMIZER_AZURE_OPENAI_API_KEY") or API_KEY,
                    )
...
def get_target_client() -> AzureOpenAI | OpenAI:
    ...
    with _target_lock:
        if _target_client is None:
            # When using qwen_chat or openai_chat backend, return an OpenAI client
            from skillopt.model.backend_config import get_target_backend
            backend = get_target_backend()
            if backend in {"qwen_chat", "openai_chat"}:
                # Try to get config from various places
                base_url = os.environ.get("TARGET_AZURE_OPENAI_ENDPOINT") or ENDPOINT
                api_key = os.environ.get("TARGET_AZURE_OPENAI_API_KEY") or API_KEY or "ollama"
                
                # If using qwen_chat, it might have its own base_url
                if backend == "qwen_chat":
                    try:
                        from skillopt.model import qwen_backend as _qwen
                        base_url = _qwen.BASE_URL or base_url
                        api_key = _qwen.API_KEY or api_key
                    except ImportError:
                        pass
