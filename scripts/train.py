...
def load_config(args: argparse.Namespace) -> dict:
    ...
    if not flat.get("out_root"):
        ...
        opt_model_cfg = flat.get("optimizer_model", "unknown")
        if isinstance(opt_model_cfg, dict):
            model_name = opt_model_cfg.get("model", "unknown")
        else:
            model_name = str(opt_model_cfg)
        model_name = model_name.replace("/", "-")
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        flat["out_root"] = os.path.join("outputs", f"skillopt_{env}_{model_name}_{ts}")
